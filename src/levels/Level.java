...
import powerUps.LazerShot;
import powerUps.LongShot;
import powerUps.PowerUps;
import powerUps.TripleShot;
import tiles.Tile;
...
public class Level {
	...
	public List<PowerUps> livePowerUps = new ArrayList<PowerUps>();
...
//		}
	...
	public void generateLevel() {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (y == 0) {
					tiles[x + y * width] = Tile.VOID.getId();
				}else if (x == 0 || y == 1 || x == width - 1 || y == height - 1) {
...
//		}
		...
		//add HUDs
		blackBar = new BlackBar(gsm);
		huds.push(blackBar);
		
		lifeBar = new LifeBar(gsm, 4, 0);
		huds.push(lifeBar);	

		for (int i = 1; i < player.getPlayerHealth(); i++) {
			lifeBarFiller = new LifeBarFiller(gsm, 2 + i, 0);
		...
		enemiesLeft = new EnemiesLeft(gsm, this);
		huds.push(enemiesLeft);
		
		gameOverHUD = new GameOver(gsm, player, this);
		huds.add(gameOverHUD);
		
		//add PowerUps
		longShot = new LongShot(0, this, "LongShot", xNum() * 8, yNum() * 8);
		powerUps.add(longShot);
		
		lazerShot = new LazerShot(0, this, "LazerShot", xNum() * 8, yNum() * 8);
		powerUps.add(lazerShot);
		
		tripleShot = new TripleShot(0, this, "TripleShot", xNum() * 8, yNum() * 8);
	...
	public void tick() {
		...
		for (PowerUps p : livePowerUps) {
			p.tick();
		}

		double startingTiles = width * height - (width * 3 +  height * 2);
		...
		if (player.isShooting) {
			...
			if (System.currentTimeMillis() - lastShotFired > fireRate) {
				...
				if (tripleShot.isActive()) {
					int shotX2 = shotX + 5;
					int shotX3 = shotX - 5;
					int shotY2 = shotY + 5;
					int shotY3 = shotY - 5;
					
					shot2 = new Shot(this, "shot2", shotX2, shotY2, 6, shotDirection, fireDistance);
					shots.add(shot2);
				
					shot3 = new Shot(this, "shot3", shotX3, shotY3, 6, shotDirection, fireDistance);
					shots.add(shot3);
				}
...
*/					addLiveMower(spareMowers.pop());							
		...
		Iterator<PowerUps> livePowerUpsIterator = livePowerUps.iterator();
		while(livePowerUpsIterator.hasNext()) {
			PowerUps livePowerUp = livePowerUpsIterator.next();
			if (livePowerUp.isUsed()) { 
				try {
					livePowerUpsIterator.remove();
				} catch (ConcurrentModificationException e) {
					e.printStackTrace();
				}
			}
...
//			Font.render("GAME OVER", screen, 50, 50, Colors.get(-1, 555, 555, 555), 1);
	...
	public int getLiveMowerNum() {
		return liveMowers.size();
	}
	
	public int getMowerNum() {
		return liveMowers.size() + spareMowers.size();
	}
}
