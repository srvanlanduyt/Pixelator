 ...
 *			>>>>>>>>>>>>>>>>>>>>COMMITTED ON GITHUB<<<<<<<<<<<<<<<<<<<<<<<<<<
 *
 *		version 2.8 - 10/19/19
 *			There is now a more gradual increase in difficulty, but it will still need to be tweaked.  There is also a mechanism that
 *			allows powerUps to be put in place.  Right now the only one that is working is the longShot, but I'll need it add in 
 *			an increased fire rate and triple shot among others. 
 *
 *			After adding the other 2 powerUps mentioned above the next steps are to implement the health PUs and then get into level 
 *			design.  These files are going to need to get straightened out soon also.  They are getting pretty ugly.  
 *		
 *		version 2.9 - 10/21/19
 *			Added a persistent black bar to the top that will keep the HUDs on a black background.  There is still an issue with 
 *			screen tear sometimes, but I don't know what is causing it yet. The level height has been shrunk by 1 to accommodate 
 *			the black bar, so now the STONE layer on the top is at y = 1. 
 *
 *		version 2.91 - 10/21/19
 *			Added "enemiesLeft" to the HUD and did a little rearranging. 
 *
 */
...
private static final long serialVersionUID = 1L;
	...
	public static final double VERSION_NUM = 2.91; 
